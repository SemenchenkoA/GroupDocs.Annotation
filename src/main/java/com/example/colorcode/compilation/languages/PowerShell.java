...
public class PowerShell implements ILanguage {
    ...
    public final IGenericList<LanguageRule> getRules() {
        ...
        tmp0.addItem(new LanguageRule("\\$(?:[\\d\\w\\-]+(?::[\\d\\w\\-]+)?|\\$|\\?|\\^)", tmp2));
        tmp0.addItem(new LanguageRule("\\${[^}]+}", tmp2));
        tmp0.addItem(new LanguageRule("\\b(begin|break|catch|continue|data|do|dynamicparam|elseif|else|end|exit|filter|finally|foreach|for|from|function|if|in|param|process|return|switch|throw|trap|try|until|while)\\b", tmp2));
        tmp0.addItem(new LanguageRule("-(?:c|i)?(?:eq|ne|gt|ge|lt|le|notlike|like|notmatch|match|notcontains|contains|replace)", tmp2));
        tmp0.addItem(new LanguageRule("-(?:band|and|as|join|not|bxor|xor|bor|or|isnot|is|split)", tmp2));
        tmp0.addItem(new LanguageRule("(?:\\+=|-=|\\*=|/=|%=|=|\\+\\+|--|\\+|-|\\*|/|%)", tmp2));
        tmp0.addItem(new LanguageRule("(?:\\>\\>|2\\>&1|\\>|2\\>\\>|2\\>)", tmp2));
        tmp0.addItem(new LanguageRule("(?s)\\[(CmdletBinding)[^\\]]+\\]", tmp2));
        tmp0.addItem(new LanguageRule("(\\[)([^\\]]+)(\\])(::)?", tmp2));
        return tmp0;
    }
